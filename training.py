...
class ChatBot:
    ...
    def main(self):
        ...
        with tf.Graph().as_default():
            ...
            # Saver/summaries
            out_dir = os.path.abspath(os.path.join(os.path.curdir, self.args.corpus_name))
    ...
    def main_test_interactive(self):
        ...
        print('测试: 加载交互模式')
        print('')
        print('欢迎来到交互模式, 我是robot；请输入您想问的问题，我会尽力回答；输入 \'exit\' 或按下 ENTER 来退出系统。玩的愉快！')
        ...
        while True:
            ...
            if not answer:
                print('很抱歉: 问句对3岁的我来说太长了，请问一个简短的句子')
    ...
    def managePreviousModel(self):
        ...
        model_path = os.path.join(os.path.curdir, self.args.corpus_name)

        ckpt = tf.train.latest_checkpoint(model_path)
        if ckpt:
            print('从{} 调用先前训练好的模型'.format(ckpt))
    ...
    def save_session(self, sess, step):
        ...
        model_name = os.path.join(self.args.corpus_name, 'model.ckpt')
